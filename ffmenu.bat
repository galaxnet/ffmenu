...
ECHO 19 - Better Deshake Video
ECHO 14 - OPTIONS MENU
ECHO E - EXIT
ECHO.
SET /P M=Type a number then press ENTER:
IF %M%==1 GOTO EXTRACTAUDIO
IF %M%==2 GOTO EXTRACTPNG
IF %M%==3 GOTO NORMAL
IF %M%==4 GOTO ADDAUDIO 
IF %M%==5 GOTO COMBINEAUDIO
IF %M%==6 GOTO REMOVEAUDIO
IF %M%==7 GOTO COMBINEMP4AUDIO
IF %M%==8 GOTO JOINALLMP4
IF %M%==9 GOTO EXRACTJPG
IF %M%==10 GOTO MP4SLIDESHOW
IF %M%==11 GOTO JPEGSLIDESHOW
IF %M%==12 GOTO SCALINGMENU
IF %M%==14 GOTO OPTIONSMENU
IF %M%==15 GOTO CROSSFADE 
IF %M%==16 GOTO ADDCREDITS
IF %M%==17 GOTO BUMPGEN
IF %M%==18 GOTO DESHAKE
IF %M%==19 GOTO BETTERDESHAKE
IF %M%==E GOTO EOF
:OPTIONSMENU
CLS
ECHO OPTIONS MENU
...
ECHO.
SET /P M=Type a number then press ENTER:
IF %M%==1 COLOR 4E & GOTO OPTIONSMENU
IF %M%==2 COLOR 5B & GOTO OPTIONSMENU
IF %M%==3 COLOR 1F & GOTO OPTIONSMENU
IF %M%==4 COLOR 8E & GOTO OPTIONSMENU
IF %M%==5 COLOR 06 & GOTO OPTIONSMENU
IF %M%==6 COLOR 0A & GOTO OPTIONSMENU
IF %M%==7 GOTO MAINMENU
IF %M%==8 
IF %M%==9 
IF %M%==10 
IF %M%==11 
IF %M%==12
IF %M%==99 GOTO OPTIONSMENU
IF %M%==E GOTO EOF
:SCALINGMENU
ECHO SCALING MENU
ECHO.
...
ECHO.
SET /P M=Type a number then press ENTER:
IF %M%==1 ffmpeg -i %* -vf scale=1920x1080:flags=lanczos output_1080p.mp4 & GOTO MENU
IF %M%==2 ffmpeg -i %* -vf scale=1280x720:flags=lanczos output_720p.mp4 & GOTO MENU
IF %M%==3 ffmpeg -i %* -vf scale=960x720:flags=lanczos output_720.mp4 & GOTO MENU
IF %M%==4 ffmpeg -i %* -vf scale=640x480:flags=lanczos output_640.mp4 & GOTO MENU 
IF %M%==5 ffmpeg -i %* -vf scale=480x360:flags=lanczos output_360.mp4 & GOTO MENU
IF %M%==6 ffmpeg -i %* -vf scale=640x360:flags=lanczos output_360p.mp4 & GOTO MENU
